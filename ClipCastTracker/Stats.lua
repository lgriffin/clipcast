...
function ns:InitDB()
    if not ClipCastCharDB then
        ClipCastCharDB = {}
    end
    local db = ClipCastCharDB
...
function ns:GetConfig()
    return ClipCastCharDB and ClipCastCharDB.config or DB_DEFAULTS.config
end

function ns:SaveConfig()
    if not ClipCastCharDB then return end
    local f = ns.alertFrame
    if f then
        local point, _, _, x, y = f:GetPoint()
        ClipCastCharDB.config.alertPoint = { point, x, y }
    end
    local w = _G["CCTMainFrame"]
    if w then
        local point, _, _, x, y = w:GetPoint()
        ClipCastCharDB.config.windowPoint = { point, x, y }
        ClipCastCharDB.config.windowWidth = w:GetWidth()
        ClipCastCharDB.config.windowHeight = w:GetHeight()
    end
end
...
function ns:SaveSession()
    ...
    for spellID, entry in pairs(sessionStats.spells) do
        local saved = ClipCastCharDB.spells[spellID]
        if not saved then
            saved = { name = entry.name, casts = 0, cancels = 0 }
            ClipCastCharDB.spells[spellID] = saved
        end
        saved.name = entry.name
        saved.casts = saved.casts + entry.casts
        saved.cancels = saved.cancels + entry.cancels
        if entry.nextCasts then
            if not saved.nextCasts then saved.nextCasts = {} end
            for nextID, nc in pairs(entry.nextCasts) do
                local snc = saved.nextCasts[nextID]
                if not snc then
                    snc = { name = nc.name, count = 0 }
                    saved.nextCasts[nextID] = snc
                end
                snc.name = nc.name
                snc.count = snc.count + nc.count
            end
        end
    end

    table.insert(ClipCastCharDB.sessions, {
        timestamp = time(),
    ...
    while #ClipCastCharDB.sessions > MAX_SESSIONS do
        table.remove(ClipCastCharDB.sessions, 1)
    end
end
...
function ns:OnCastStart(cast)
    ...
    if ns.RefreshMainWindow then ns:RefreshMainWindow() end
end
...
function ns:ResetLifetime()
    if not ClipCastCharDB then return end
    ClipCastCharDB.spells = {}
    ClipCastCharDB.sessions = {}
    if ns.RefreshMainWindow then ns:RefreshMainWindow() end
end
...
function ns:GetLifetimeStats()
    ...
    local merged = {}
    local seen = {}
    for spellID, saved in pairs(ClipCastCharDB.spells) do
        local session = sessionStats.spells[spellID]
        local nextCasts = {}
        if saved.nextCasts then
            for nextID, nc in pairs(saved.nextCasts) do
                nextCasts[nextID] = { name = nc.name, count = nc.count }
            end
        end
        if session and session.nextCasts then
            for nextID, nc in pairs(session.nextCasts) do
                if nextCasts[nextID] then
                    nextCasts[nextID].count = nextCasts[nextID].count + nc.count
                    nextCasts[nextID].name = nc.name
                else
                    nextCasts[nextID] = { name = nc.name, count = nc.count }
                end
            end
    ...
    for spellID, entry in pairs(sessionStats.spells) do
        if not seen[spellID] then
            table.insert(merged, {
                ...
                nextCasts = entry.nextCasts or {},
            })
        end
    end
    table.sort(merged, function(a, b)
        if a.cancels ~= b.cancels then return a.cancels > b.cancels end
...
function ns:GetSessionHistory()
    if not ClipCastCharDB then return {} end
    return ClipCastCharDB.sessions
end

function ns:RecordNextCast(cancelSpellID, nextSpellID, nextName)
    local entry = sessionStats.spells[cancelSpellID]
    if not entry then return end
    if not entry.nextCasts then entry.nextCasts = {} end
    local nc = entry.nextCasts[nextSpellID]
    if not nc then
        nc = { name = nextName, count = 0 }
        entry.nextCasts[nextSpellID] = nc
    end
    nc.count = nc.count + 1
    if ns.RefreshMainWindow then ns:RefreshMainWindow() end
end

function ns:GetSessionSpellList()
    local list = {}
    for spellID, entry in pairs(sessionStats.spells) do
        table.insert(list, {
            spellID = spellID,
...
function ns:GetDrilldownData(spellID, isLifetime)
    local nextCasts = {}
    if isLifetime then
        local saved = ClipCastCharDB and ClipCastCharDB.spells[spellID]
        if saved and saved.nextCasts then
            for nextID, nc in pairs(saved.nextCasts) do
                nextCasts[nextID] = { name = nc.name, count = nc.count }
            end
        end
        ...
        local entry = sessionStats.spells[spellID]
        if entry and entry.nextCasts then
            for nextID, nc in pairs(entry.nextCasts) do
                nextCasts[nextID] = { name = nc.name, count = nc.count }
            end
        end
    ...
    local list = {}
    for nextID, nc in pairs(nextCasts) do
        table.insert(list, { spellID = nextID, name = nc.name, count = nc.count })
    end
